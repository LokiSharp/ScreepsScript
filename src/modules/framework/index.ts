 ...
 * 默认的 bot 名称
 */
const DEFAULT_BOT_NAME = `LokiSharp${BOT_NAME_SUFFIX}`;
...
export default class App {
   ...
   */
  public constructor({ name, mountList }: CreateOptions = {}) {
   ...
   */
  public set catcher(newCatcher: ErrorCatcher) {
  ...
  public close(deleteTarget: number): App {
    ...
    Object.values(this.lifecycleCallbacks).map(callbackList => {
      ...
      callbackList.find(({ id }, index) => {
        if (id !== deleteTarget) return false;

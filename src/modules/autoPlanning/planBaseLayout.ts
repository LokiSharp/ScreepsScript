...
export const layout = {
   ...
   */
  get(centerFlagName: string, baseSize = 11): string {
    // 没有存储就新建
    ...
    allStructure.forEach(s => {
      ...
      // 范围外的就当作位置动态计算的建筑
      const relativePos: [number, number] = s.pos.inRangeTo(flag, baseSize / 2 - 0.5)
        ? [s.pos.x - flag.pos.x, s.pos.y - flag.pos.y]
        : null;
    ...
    Object.keys(layout).forEach((structureType: StructureConstant) => {
      // 计算该种类建筑在之前出现过多少次
      const preLength = _.sum(
        Object.values(Memory.layoutInfo).map(levelLayout =>
          structureType in levelLayout ? levelLayout[structureType].length : 0
        )
      );
  ...
  if (type === STRUCTURE_LINK) {
    ...
    for (const target of targets) {
      ...
      if (
        ...
        target.pos.findInRange(FIND_MY_CONSTRUCTION_SITES, 2, { filter: s => s.structureType === STRUCTURE_LINK })
          .length > 0
      )
        continue;
...
export const planLayout = function (room: Room): OK | ERR_NOT_OWNER | ERR_NOT_FOUND {
  ...
  // 当前需要检查那几个等级的布局
  const planLevel = Array(room.controller.level)
    .fill(undefined)
    .map((_, index) => index + 1);
  // 房间保存的中心位置

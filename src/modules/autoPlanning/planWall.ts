 ...
 */
const getOutWalls = function (
  impassableCostMatrix: CostMatrix,
  ...
  // 遍历所有位置进行蔓延，找到目标墙壁位置
  walkedTile.forEach(
    spread(impassableCostMatrix, (pos, posName) => {
      // 距离够了就停止蔓延并加入最终结果
 ...
 */
const getNeededWalls = function (
  room: Room,
  impassableCostMatrix: CostMatrix,
  outWalls: RoomPosition[]
): [RoomPosition[], CostMatrix] {
  // 生成字符串标记，用于后面进行对比
 ...
 */
const getThickerWalls = function (
  impassableCostMatrix: CostMatrix,
  neededWalls: RoomPosition[],
  thickness: number
): RoomPosition[][] {
 ...
 * @returns 需要建墙的位置
 */
export default function (room: Room, centerPos: RoomPosition, distanceToCenter = 7, thickness = 3): RoomPosition[][] {

 ...
 * @param data 任务数据
 */
function serializeTask<K extends AllDelayTaskName>(name: K, data: DelayTaskTypes[K]): string {
 ...
 */
function unserializeTask(taskString: string): [AllDelayTaskName, DelayTaskTypes[AllDelayTaskName]] {
  const [name, ...tasks] = taskString.split(" ");
  return [name as AllDelayTaskName, JSON.parse(tasks.join(" ")) as DelayTaskData];
}
 ...
 */
export function addDelayCallback<K extends AllDelayTaskName>(name: K, callback: DelayTaskCallback<K>): void {
 ...
 */
export function manageDelayTask(): void {

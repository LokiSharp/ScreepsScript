...
export const DelayQueue = function (): {
  saveDelayTasks: () => void;
  addDelayTask: <K extends AllDelayTaskName>(name: K, data: DelayTaskTypes[K], call: number) => void;
  manageDelayTask: () => void;
  addDelayCallback: <K extends AllDelayTaskName>(name: K, callback: DelayTaskCallback<K>) => void;
  initDelayTasks: () => void;
} {
  /**
   * 所有的任务回调都会被存放到这里
   ...
   */
  const addDelayTask = function <K extends AllDelayTaskName>(name: K, data: DelayTaskTypes[K], call: number): void {
    // 不能添加过去的延迟任务
   ...
   * 需要在全局重置时调用
   */
  const initDelayTasks = function (): void {
    if (!Memory[SAVE_KEY]) return;
    delayTasks = JSON.parse(Memory[SAVE_KEY]) as { [callTime: number]: DelayTask[] };
  };
   ...
   */
  const addDelayCallback = function <K extends AllDelayTaskName>(name: K, callback: DelayTaskCallback<K>): void {

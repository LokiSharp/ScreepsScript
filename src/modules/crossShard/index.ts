...
const initShardData = function (): void {
  ...
  ALL_SHARD_NAME.forEach(name => {
    ...
    if (selfShardName === name && selfData) {
      selfData = (JSON.parse(InterShardMemory.getLocal()) || {}) as InterShardData;
    } else {
      // 重建其他 shard 的数据
  ...
  for (const shardName in otherShardData) {
    ...
    for (const msgName in otherShardData[shardName]) {
      ...
      // 该信息是请求
        ...
        const interShardData: InterShardData = otherShardData[shardName as ShardName];
        const request: CrossShardRequest = interShardData[msgName] as CrossShardRequest;

        if (request.to === selfShardName) {
          if (!request.type || !requestHandleStrategies[request.type])
            return log(`未知跨 shard 请求，不予执行 ${request.type}`, ["跨 shard"], "yellow");

 ...
 * @returns 当前应该从哪里获取能量
 */
export function getRoomAvailableSource(room: Room, opt?: GetAvailableSourceOpt<true>): AllEnergySource;
export function getRoomAvailableSource(room: Room, opt?: GetAvailableSourceOpt<false>): EnergySourceStructure;
export function getRoomAvailableSource(
  room: Room,
  opt: GetAvailableSourceOpt = { includeSource: true, ignoreLimit: false }
): AllEnergySource {
  ...
  if (room[STRUCTURE_CONTAINER].length > 0) {
    ...
    // 能量必须够多才会选用
    const availableContainer = room[STRUCTURE_CONTAINER].filter(
      container => container.store[RESOURCE_ENERGY] > useLimit
    );
    // 挑个能量多的 container
  ...
  // 没有就选边上有空位的 source
  return opt.includeSource
    ? room.source.find(source => {

 ...
 */
export const getRoomEnergyTarget = function (
  room: Room,
  finder?: EnergyTargetFinder,
  ...
  if (!allEnergyTargets) {
    ...
    const structureTargets = [room.storage, room.terminal, ...room[STRUCTURE_CONTAINER]].filter(
      structure => structure && structure.store[RESOURCE_ENERGY] > 0
    );

    // 查找 source 旁边地上扔的
    const droppedEnergyTargets = room.source.map(source => source.getDroppedInfo().energy).filter(Boolean);

...
class CreepRelease implements InterfaceCreepRelease {
   ...
   */
  public remoteReserver(remoteRoomName: string, single = true): void {
    // 添加外矿预定者
   ...
   */
  public remoteHelper(remoteRoomName: string, wayPointFlagName?: string): void {
   ...
   */
  public pbCarrierGroup(flagName: string, num: number): void {
    // 如果已经有人发布过了就不再费事了
   ...
   * @param keepSpawn 是否持续生成
   */
  public dismantleGroup(targetFlagName = "", keepSpawn = false): string {
    const room = Game.rooms[this.roomName];
    if (!room) return `错误，无法访问的房间 ${this.roomName}`;
    ...
    const healerName = `${room.name} boostHealer ${Game.time}`;
    creepApi.add(
      dismantlerName,
      "boostDismantler",
      {
  ...
  public attacker(targetFlagName = "", num = 1, keepSpawn = false): string {
    ...
    for (let i = 0; i < num; i++) {
      creepApi.add(
        `${this.roomName} attacker ${Game.time}-${i}`,
        "attacker",
        {
          targetFlagName: targetFlagName || DEFAULT_FLAG_NAME.ATTACK,
          keepSpawn
   ...
   * @param keepSpawn 是否持续生成
   */
  public dismantler(targetFlagName = "", num = 2, keepSpawn = false): string {
    if (num <= 0 || num > 10) num = 1;

    for (let i = 0; i < num; i++) {
      creepApi.add(
        `${this.roomName} dismantler ${Game.time}-${i}`,
        "dismantler",
        {
          targetFlagName: targetFlagName || DEFAULT_FLAG_NAME.ATTACK,
          keepSpawn
 ...
 */
export default function (key = "release"): void {
  Object.defineProperty(Room.prototype, key, {
    get() {
      if (!((this as Room).name in releaseControllers)) {
        releaseControllers[(this as Room).name] = new CreepRelease((this as Room).name);
      }

      return releaseControllers[(this as Room).name];
    },
    enumerable: false,
    configurable: true
  });

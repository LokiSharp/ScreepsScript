...
interface InterfaceCreepRelease {
   ...
   * 孵化 pbCarrier 小组
   */
  pbCarrierGroup(flagName: string, num: number): void;
  /**
   * 孵化 pb 采集小组（一红一绿为一组）
   */
   ...
   * 孵化进攻一体机
   */
  rangedAttacker(targetFlagName?: string, num?: number, keepSpawn?: boolean): string;
  /**
   * 孵化基础进攻单位
   */
  attacker(targetFlagName?: string, num?: number, keepSpawn?: boolean): string;
  /**
   * 孵化基础拆除单位
   * 一般用于清除中立房间中挡路的墙壁
   */

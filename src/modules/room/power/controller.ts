import { MAX_OPS, PWR_ENABLE_ROOM } from "./constant";
...
export default class RoomPowerController extends RoomAccessor<PowerConstant[]> {
  ...
  ): OK | ERR_NAME_EXISTS | ERR_INVALID_TARGET | ERR_NO_BODYPART {
  ...
  public addSkill(pc: PowerCreep): void {
    const newPowers = Object.keys(pc.powers).map(power => Number(power)) as PowerConstant[];

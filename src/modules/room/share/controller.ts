...
export default class RoomShareController extends RoomAccessor<RoomShareTask> {
   ...
   * 允许设置局部值，不允许覆盖
   */
  private static get resourceSource(): ResourceSourceMap {
  ...
  public becomeSource(resourceType: ResourceConstant): boolean {
    if (!(resourceType in RoomShareController.resourceSource)) RoomShareController.resourceSource[resourceType] = [];

    const alreadyRegister = RoomShareController.resourceSource[resourceType].find(name => name === this.roomName);
    // 如果已经被添加过了就返回 false
    if (alreadyRegister) return false;

    RoomShareController.resourceSource[resourceType].push(this.roomName);
    return true;
  }
  ...
  public leaveSource(resourceType: ResourceConstant): void {
    // 没有该资源就直接停止
    if (!(resourceType in RoomShareController.resourceSource)) return;

    // 获取该房间在资源来源表中的索引
    _.pull(RoomShareController.resourceSource[resourceType], this.roomName);
    // 列表为空了就直接移除
    if (RoomShareController.resourceSource[resourceType].length <= 0)
      delete RoomShareController.resourceSource[resourceType];
  }
  ...
  public handle(targetRoom: string, resourceType: ResourceConstant, amount: number): boolean {
    ...
    const freeSpace =
      selfRoom.terminal.store.getFreeCapacity() +
      terminalAmount -
  ...
  private getSource(resourceType: ResourceConstant): Room | null {
    const SourceRoomsName = RoomShareController.resourceSource[resourceType];
    if (!SourceRoomsName) return null;

    // 寻找合适的房间
    let targetRoom: Room;
    // 变量房间名数组，注意，这里会把所有无法访问的房间筛选出来
    RoomShareController.resourceSource[resourceType] = SourceRoomsName.filter(roomName => {

...
export default class RoomAccessor<MemoryType> {
   ...
   */
  public constructor(moduleName: string, roomName: string, memoryKey: string, defaultMemory: MemoryType) {
  ...
  protected get memory(): MemoryType {
    ...
    try {
      ...
      log(`无法访问房间实例，模块已停止运行`, [this.moduleName, this.roomName], "red", true);
      throw e;
    }
  }
  ...
  protected saveMemory(): void {
    try {
      ...
      // 否则将内存格式化成字符串进行保存
        ...
        Game.rooms[this.roomName].memory[this.memoryKey] = JSON.stringify(this._memory);
      }
    } catch (e) {
      log(`无法访问房间实例，模块已停止运行`, [this.moduleName], "red", true);
      throw e;
   ...
   * @param notify 是否发送邮件
   */
  protected log(content: string, color: Colors | undefined = undefined, notify = false): void {
    // 为房间名添加超链接

...
describe("creep 发布核心测试", () => {
  ...
  it("harvester 发布测试", () => {
    ...
    expect(
      mockAddTask.mock.calls.map(([spawnTask]: SpawnTask[]) => (spawnTask.data as HarvesterData).sourceId)
    ).toEqual(["1", "2"]);
  });
  ...
  it("changeBaseUnit 可以正常增减，并保证最小单位数量", () => {
    ...
    // 会新增四个工人
    expect(mockAddTask.mock.calls.map(([spawnTask]: SpawnTask[]) => spawnTask.name)).toEqual([
    ...
    // 从末尾减少了一个 worker
    expect(mockRemoveCreep.mock.calls.map(([creepName]: string[]) => creepName)).toEqual([GetName.worker("W1N1", 3)]);
    ...
    expect(mockAddTask).not.toHaveBeenCalled();
    expect(mockRemoveCreep.mock.calls.map(([creepName]: string[]) => creepName)).toEqual([
      GetName.worker("W1N1", 2),
      GetName.worker("W1N1", 1)
    ]);
  });
  ...
  it("setBaseUnitLimit 重设数量上下限后，可以自动矫正数量", () => {
    ...
    expect(mockAddTask.mock.calls.length).toEqual(10);
  });
});
